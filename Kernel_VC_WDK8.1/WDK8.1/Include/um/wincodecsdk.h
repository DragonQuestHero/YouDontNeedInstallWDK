...
HRESULT WINAPI WICMatchMetadataContent(
    _In_reads_(1) REFGUID guidContainerFormat,
    _In_reads_opt_(1)  const GUID *pguidVendor,
    _In_reads_(1)  IStream *pIStream,
    _Out_writes_(1) GUID *pguidMetadataFormat
    );
HRESULT WINAPI WICSerializeMetadataContent(
    _In_reads_(1) REFGUID guidContainerFormat,
    _In_reads_(1) IWICMetadataWriter *pIWriter,
...
HRESULT WINAPI WICGetMetadataContentSize(
    _In_reads_(1) REFGUID guidContainerFormat,
    _In_reads_(1) IWICMetadataWriter *pIWriter,
    _Out_writes_(1) ULARGE_INTEGER *pcbSize
    );
