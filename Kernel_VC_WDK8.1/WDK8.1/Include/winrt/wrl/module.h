...
_Ret_range_(<=, end - first) inline unsigned int CountObjectEntries(_In_ _In_range_(<=, end) const CreatorMap** first, _In_ const CreatorMap** end, _In_opt_z_ const wchar_t* serverName) throw()
{
...
inline HRESULT STDMETHODCALLTYPE CreateClassFactory(_In_ unsigned int *flags, _In_ const CreatorMap*, REFIID riid, _Outptr_ IUnknown **ppFactory) throw()
{
    ...
    switch(Factory::cacheFlag)
    {
        case FactoryCacheFlags::FactoryCacheEnabled:
            if ((*flags & DisableCaching) != 0)
            {
                __WRL_ASSERT__("Mismatched Module<> and 'Factory' configuration. 'Factory' is cachable and Module<> doesn't support caching");
...
public:
    STDMETHOD_(unsigned long, IncrementObjectCount)()
    {
        if (count_ != nullptr)
        {
            ...
            return __super::IncrementObjectCount();
        }
    ...
    STDMETHOD_(unsigned long, DecrementObjectCount)()
    {
        ...
        if (count_ != nullptr)
        {
            ...
            ref = __super::DecrementObjectCount();
            if (ref == 0)
            {
